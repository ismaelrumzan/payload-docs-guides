...
import type { Guide, Config } from '@/payload-types'
...
export const getGuides = cache(
  async (forceDraft?: boolean, locale?: Config['locale']): Promise<Guide[]> => {
    try {
...
export const getGuideBySlug = cache(
  async (slug: string, forceDraft?: boolean): Promise<Guide | null> => {
    if (!slug) return null
...
export const getGuideById = cache(
  async (
    id: string | number,
    forceDraft?: boolean,
